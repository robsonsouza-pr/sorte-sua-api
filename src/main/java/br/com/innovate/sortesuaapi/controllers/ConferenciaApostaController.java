...
public class ConferenciaApostaController {
	...
	@PostMapping
	public ResponseEntity<Response<ConferenciaApostaDto>> conferir (@Valid @RequestBody ConferenciaApostaDto conferenciaApostaDto, BindingResult result){
	...
	private Resultado validarConferencia(ConferenciaApostaDto conferenciaApostaDto, BindingResult result) {
		...
		resultado = resultadoService.buscarPorNumeroETipoSorteio(conferenciaApostaDto.getIdLoteria(), conferenciaApostaDto.getNumeroSorteio());
		if(resultado== null) 	{
			result.addError(new ObjectError("Não foi enconrado resultado cadastrado para o sorteio {}", conferenciaApostaDto.getNumeroSorteio()) );
			return resultado;
		}
		 ...
		 }else if(!dezenas.isEmpty() && ! DezenaUtils.isRangeApostaValido(dezenas.size(), conferenciaApostaDto.getIdLoteria())) {
